package tw.haotek.command.HaotekW.device;

import android.util.Log;

import org.xmlpull.v1.XmlPullParser;
import org.xmlpull.v1.XmlPullParserException;
import org.xmlpull.v1.XmlPullParserFactory;

import java.io.IOException;
import java.io.StringReader;

import tw.haotek.command.HaotekW.HaotekCommand;
import tw.haotek.command.WiFiCommandDefine;
import tw.haotek.dut.HaotekDevice;
import tw.haotek.dut.data.DeviceInfo;

/**
 * Created by Neo on 2015/12/9.
 */
public class GetDeviceInfoCommand extends HaotekCommand {
    private static final String TAG = GetDeviceInfoCommand.class.getSimpleName();

    public GetDeviceInfoCommand(HaotekDevice device, int CommandType) {
        super(device, CommandType);
    }

    @Override
    protected String getAction() {
//        return "?custom=1&cmd=9003";
        return "?custom=1&cmd=" + WiFiCommandDefine.WIFIAPP_CMD_GET_DEVICE_INFO;
    }

    public static class Response extends HaotekCommand.Response {
        public DeviceInfo mList = new DeviceInfo();
    }

    protected Response dispatchResponse(String soap) {
        super.dispatchResponse(soap);
        Response response = new Response();
        Log.d(TAG, "Soap : " + soap);
        DeviceInfo list = null;
        try {
            XmlPullParserFactory factory = XmlPullParserFactory.newInstance();
            factory.setNamespaceAware(true);
            XmlPullParser xpp = factory.newPullParser();
            xpp.setInput(new StringReader(soap));
            int eventType = xpp.getEventType();
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tagName = xpp.getName();
                    Log.d(TAG, " tagName : " + tagName);
                    switch (tagName) {
                        case "FirmwareVersion":
                            list = new DeviceInfo();
                            list.mFirmwareVersion = xpp.nextText();
                            break;
                        case "Model":
                            list.mModel = xpp.nextText();
                            break;
                        case "MAC":
                            list.mMAC = xpp.nextText();
                            break;
                        case "UID":
                            list.mUID = xpp.nextText();
                            break;
                        case "STREAMTYPE":
                            list.mMode = xpp.nextText();
                            break;
                    }
                }
                eventType = xpp.next();
            }
            response.mList = list;
        } catch (XmlPullParserException e) {
            e.printStackTrace();
        } catch (IOException e) {
            e.printStackTrace();
        }
        return response;
    }
}
