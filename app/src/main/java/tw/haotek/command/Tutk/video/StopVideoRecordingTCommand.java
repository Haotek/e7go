 ...
 */
public class StopVideoRecordingTCommand extends TutkCommand {
    private static final String TAG = StopVideoRecordingTCommand.class.getSimpleName();
    private int mPar;

    public StopVideoRecordingTCommand(HaotekDevice device, int par) {
        super(device);
        mPar = par;
    }
...
//        mAgent.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_WIFICMD_REQ, Convert.sendWiFiCmdRequest(0, 0, 0, getActionID(), 1, 0, getAction().length(), getAction()));
        mAgent.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SETRECORD_REQ, AVIOCTRLDEFs.SMsgAVIoctrlSetRecordReq.parseContent(0, AVIOCTRLDEFs.AVIOTC_RECORDTYPE_OFF));
