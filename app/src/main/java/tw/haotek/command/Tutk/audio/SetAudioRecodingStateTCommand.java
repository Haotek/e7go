package tw.haotek.command.Tutk.audio;
 ...
 */
public class SetAudioRecodingStateTCommand extends TutkCommand {
    private static final String TAG = SetAudioRecodingStateTCommand.class.getSimpleName();
    private int mPar;

    public SetAudioRecodingStateTCommand(HaotekDevice device, int par) {
        super(device);
        mPar = par;
    }
    ...
    protected String getAction() {
        Log.d(TAG, "Show Action : " + "custom=1&cmd=" + getActionID() + "&par=" + mPar);
        return "custom=1&cmd=" + getActionID() + "&par=" + mPar;
    }

    @Override
    protected int getActionID() {
        return WiFiCommandDefine.MOVIE_AUDIO;
    }
